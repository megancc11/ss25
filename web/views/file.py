...
def file(request, project_id):
    ...
    parent_object = None#父目录
    folder_id = request.GET.get('folder', "")#是否有根目录
    if folder_id.isdecimal():#是否是数字
    ...
    if request.method == "GET":
        ...
        while parent:
            # breadcrumb_list.insert(0, {'id': parent.id, 'name': parent.name})，代码效果一样
            breadcrumb_list.insert(0, model_to_dict(parent, ['id', 'name']))#目录循环，先找最底层文件夹
            parent = parent.parent

        # 当前目录下所有的文件 & 文件夹获取到即可
        queryset = models.FileRepository.objects.filter(project=request.tracer.project)#条件分开了写，当前项目下所有目录
        if parent_object:
            # 进入了某子目录
            file_object_list = queryset.filter(parent=parent_object).order_by('-file_type')#倒叙排列
        else:
            # 根目录
            file_object_list = queryset.filter(parent__isnull=True).order_by('-file_type')#parent__isnull=True,parent字段为空
        form = FolderModelForm(request, parent_object)

        context = {
            'form': form,#新建表单
            "file_object_list": file_object_list,#文件对象
            "breadcrumb_list": breadcrumb_list,#导航条
            'folder_object': parent_object  #当前目录
        }
    ...
    if fid.isdecimal():
        edit_object = models.FileRepository.objects.filter(id=int(fid), file_type=2,
                                                           project=request.tracer.project).first()#获取要编辑的对象
...
def file_upload_test(request,project_id):
    """测试js直接上传"""
    return render(request, 'web/file_upload_test2.html')
...
def file_delete(request, project_id):
    ...
    for folder in folder_list:
        child_list = models.FileRepository.objects.filter(project=request.tracer.project, parent=folder).order_by('-file_type')# 找他下面的 文件和文件夹，文件夹在前面
...
@csrf_exempt
def cos_credential_test(request, project_id):
    data_dict = credential(request.tracer.project.bucket, request.tracer.project.region)#当前项目的桶
    return JsonResponse({'status': True, 'data': data_dict})

@csrf_exempt
def cos_credential(request, project_id):
    """ 获取cos上传临时凭证 """
    #安装一个生成临时凭证的python模块，pip install qcloud-python-sts==3.1.5
    per_file_limit = request.tracer.price_policy.per_file_size * 1024 * 1024#M单位转换为B
    total_file_limit = request.tracer.price_policy.project_space * 1024 * 1024 * 1024#G单位转换为B

    total_size = 0
    file_list = json.loads(request.body.decode('utf-8'))#解析前端传回来的序列化数据
    for item in file_list:
        # 文件的字节大小 item['size'] = B
        # 单文件限制的大小 M
        # 超出限制
        if item['size'] > per_file_limit:
            msg = "文件：{}，单文件超出限制（最大{}M），请升级套餐。".format(item['name'],request.tracer.price_policy.per_file_size)
...
def file_post(request, project_id):
    ...
    if form.is_valid():
        ...
        result = {
            ...
            'download_url': reverse('file_download', kwargs={"project_id": project_id, 'file_id': instance.id})#生成URL地址，kwargs是参数
...
def file_download(request, project_id, file_id):
    ...
    res = requests.get(file_object.file_path)#获取网络文件的方法
