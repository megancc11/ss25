...
class CheckFilter(object):
    ...
    def __iter__(self):
        for item in self.data_list:
            ...
            query_dict = self.request.GET.copy()#复制一份
            query_dict._mutable = True#让request可以被修改
            query_dict.setlist(self.name, value_list)#status=1&age=19
            if 'page' in query_dict:
                query_dict.pop('page')#从字典里删除

            param_url = query_dict.urlencode()#把字典里面的内容生成URL
            if param_url:
...
def issues(request, project_id):
    if request.method == "GET":
        ...
        allow_filter_name = ['issues_type', 'status', 'priority', 'assign', 'attention']#允许的筛选条件
        condition = {}
        for name in allow_filter_name:
            value_list = request.GET.getlist(name)  # [1,2],getlist获取所有的值
            if not value_list:
                continue
        ...
        page_object = Pagination(
            ...
            base_url=request.path_info,#当前访问的路径
            query_params=request.GET,
            per_page=10
        )
        issues_object_list = queryset[page_object.start:page_object.end]#数据切片
        ...
        context = {
            ...
            #问题筛选功能
            'filter_list': [
                {'title': "问题类型", 'filter': CheckFilter('issues_type', project_issues_type, request)},#有ForeignKey
...
def issues_record(request, project_id, issues_id):
    ...
    # 判断是否可以评论和是否可以操作这个问题，暂时不做判断

    if request.method == "GET":
        reply_list = models.IssuesReply.objects.filter(issues_id=issues_id, issues__project=request.tracer.project)#issues__project关联表
        # 将queryset转换为json格式
        data_list = []
...
def issues_change(request, project_id, issues_id):
    """记录修改，涉及到的知识点orm字段信息获取"""
    issues_object = models.Issues.objects.filter(id=issues_id, project_id=project_id).first()

    post_dict = json.loads(request.body.decode('utf-8'))#获取用户递交的字典
    """
    ...
    field_object = models.Issues._meta.get_field(name)#获取表的指定字段是否为空和字段名称
    ...
    if name in ["subject", 'desc', 'start_date', 'end_date']:
        if not value:
            ...
            setattr(issues_object, name, None)#反射，设置为空
            issues_object.save()
    ...
    if name in ['issues_type', 'module', 'parent', 'assign']:
        ...
        if not value:
            ...
            if name == 'assign':
                ...
                #instance = field_object.rel.model.objects.filter(id=value, project_id=project_id).first()#新版本AttributeError: 'ForeignKey' object has no attribute 'rel'改用remote_field
                if not instance:
                    return JsonResponse({'status': False, 'error': "您选择的值不存在"})
    ...
    if name in ['priority', 'status', 'mode']:
        selected_text = None
        for key, text in field_object.choices:#field_object.choices获取枚举字段的值
            if str(key) == value:
                selected_text = text
    ...
    if name == "attention":
        ...
        if not value:
            issues_object.attention.set(value)#设置关联关系是空
            issues_object.save()
            change_record = "{}更新为空".format(field_object.verbose_name)
        else:
            # values=["1","2,3,4]  ->   id是否是项目成员（参与者、创建者）
            # 获取当前项目的所有成员
            user_dict = {str(request.tracer.project.creator_id): request.tracer.project.creator.username}#创建者
            ...
            for user_id in value:
                username = user_dict.get(str(user_id))#在列表里面查一下是否存在
                if not username:
...
def invite_url(request, project_id):
    ...
    if form.is_valid():
        ...
        if request.tracer.user != request.tracer.project.creator:
            form.add_error('period', "无权创建邀请码")#错误信息加在那个字段后面
            return JsonResponse({'status': False, 'error': form.errors})
        ...
        # 将验邀请码返回给前端，前端页面上展示出来。反向生成URL的方法
...
def invite_join(request, code):
    ...
    # 是否已过期，如果已过期则使用免费额度
    max_transaction = models.Transaction.objects.filter(user=invite_object.project.creator).order_by('-id').first()#当前用户交易记录的最大值
    if max_transaction.price_policy.category == 1:
        max_member = max_transaction.price_policy.project_member#只有免费额度
    else:#有购买记录
        if max_transaction.end_datetime < current_datetime:#购买已经过期
            free_object = models.PricePolicy.objects.filter(category=1).first()#免费额度
