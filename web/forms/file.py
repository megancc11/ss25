...
class FolderModelForm(BootStrapForm, forms.ModelForm):
    ...
    def clean_name(self):
        ...
        # 数据库判断 当前目录 下此 文件夹是否已存在，不同目录的文件夹名称可以重复
        queryset = models.FileRepository.objects.filter(file_type=2, name=name, project=self.request.tracer.project)#查询语句和条件分开来写
        if self.parent_object:
            exists = queryset.filter(parent=self.parent_object).exists()#当前子目录下是否存在重名
        else:
            exists = queryset.filter(parent__isnull=True).exists()#根目录是否存在重名
        if exists:
...
class FileModelForm(forms.ModelForm):
    etag = forms.CharField(label='ETag')#多了一个字段的校验
    ...
    def clean(self):
        ...
        try:
            ...
            self.add_error("key", '文件不存在')#添加错误不继续执行
            return self.cleaned_data
