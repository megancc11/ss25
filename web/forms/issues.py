...
class IssuesModelForm(BootStrapForm, forms.ModelForm):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        module_list.extend(module_object_list)#添加列表元素
        self.fields['module'].choices = module_list

        # 3.指派和关注者
        # 数据库找到当前项目的参与者 和 创建者
        total_user_list = [(request.tracer.project.creator_id, request.tracer.project.creator.username), ]#创建者
        project_user_list = models.ProjectUser.objects.filter(project=request.tracer.project).values_list('user_id','user__username')#参与者
